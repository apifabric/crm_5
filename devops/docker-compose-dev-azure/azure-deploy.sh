...
resourcename="crm5"  # lower case, only
resourcegroup="crm5_rg"
...
# git clone https://github.com/apilogicserver/crm5.git
# cd crm5
...
az group create --name crm5_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group crm5_rg --sku S1 --is-linux
az container create --resource-group crm5_rg --name crm5 --image apilogicserver/crm5:latest --dns-name-label crm5 --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//crm5.westus.azurecontainer.io:5656'
